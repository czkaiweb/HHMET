...
int Hbb_analysis2_0507(){
  
  TFile *fin=new TFile("/data2/weikai/calchep/delphes_results/HHMET/gYqq0.3MY1100MF300/delphes/myprocess_cms8tev.root","READONLY");
  ...
  for(Long64_t i = 0; i < nevent; i++){
	...
	if( Bool_t(JET_BTAG[j] & (1<<2)) ){
      ...
      for(int j=0; j<nJET; j++){
	if(b_vector_index==4) continue;
	if( Bool_t(JET_BTAG[j] & (1<<1)) && !Bool_t(JET_BTAG[j] & (1<<2)) ){
	  b_vector[b_vector_index].SetPtEtaPhiM(JET_PT[j],JET_ETA[j],JET_PHI[j],JET_M[j]);
	  b_vector_index++;
	}
      }
      for(int j=0; j<nJET; j++){
	if(b_vector_index==4) continue;
	if( Bool_t(JET_BTAG[j] & (1<<0)) && !Bool_t(JET_BTAG[j] & (1<<1))  && !Bool_t(JET_BTAG[j] & (1<<2)) ){
	  b_vector[b_vector_index].SetPtEtaPhiM(JET_PT[j],JET_ETA[j],JET_PHI[j],JET_M[j]);
          b_vector_index++;
	}
      }
  ...
  //Double_t fvar = fnum ;
  //Double_t kvar = nknum ;
  //ofstream fileout2("CHNAME_acc_eff.dat", ios::app);
  //fileout2 << fvar << " " << kvar << " " << cut_flow[10] << " " << cut_flow[11] << " " << cut_flow[12] << " " << cut_flow[13] << "" << endl;
  //fileout2.close();
