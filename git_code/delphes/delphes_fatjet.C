...
int delphes_analysis(char* string){
  ...
  foutname ="analysis_results/"+foutname+"_fatjet_results.root";
...
//  TFile *fin=new TFile("/data2/weikai/calchep/delphes_results/HHMET/ttbar/delphes/myprocess_cms8tev.root","READONLY");
...
//  TLorentzVector b_vector[4];
  ...
  Int_t nFATJET=-1;
  Float_t FATJET_PT[10];
  Float_t FATJET_ETA[10];
  Float_t FATJET_PHI[10];
  Float_t FATJET_M[10];
  TLorentzVector all_vector[10];
//  TLorentzVector b_vector[4];
  t->SetBranchAddress("FatJet",&nFATJET);

  TBranch *fatjet_pt  = t->GetBranch("FatJet.PT");
  fatjet_pt->SetAddress(&FATJET_PT);
  TBranch *fatjet_eta = t->GetBranch("FatJet.Eta");
  fatjet_eta->SetAddress(&FATJET_ETA);
  TBranch *fatjet_phi = t->GetBranch("FatJet.Phi");
  fatjet_phi->SetAddress(&FATJET_PHI);
  TBranch *fatjet_m = t->GetBranch("FatJet.Mass");
  fatjet_m->SetAddress(&FATJET_M);
...
//    t->GetEntry(i);
    ...
    jet_btag->GetEntry(i);
    //cout << nElec <<endl;
    fatjet_pt->GetEntry(i);
    fatjet_phi->GetEntry(i);
    fatjet_eta->GetEntry(i);
    fatjet_m->GetEntry(i);
    e_pt->GetEntry(i);
   // cout<<ELEC_PT[0]<<endl;
    e_eta->GetEntry(i);
...
//----------------ol+(4-5)jets-----------------------
   ...
   for(int l=0;l<nJET;l++){
     if(JET_PT[l]>30 && abs(JET_ETA[l])<2.4) nJET_d++;
   }
   for(int l=0;l<nFATJET;l++){
     if(FATJET_PT[l]>30 && abs(JET_ETA[l])<2.4) nJET_d++; !!!HERE
   }
   for(int l=0;l<nElec;l++){
     if(ELEC_PT[l]>10 && abs(ELEC_ETA[l])<2.5) nL_d++;
   }
   for(int l=0;l<nMUON;l++){
     if(MUON_PT[l]>10 && abs(MUON_ETA[l])<2.4) nL_d++;
   }
...
//---------------Nb,T>2---------------------------
   ...
   Int_t Nb_L=0;
   for(int l=0;l<nJET;l++){
     if(Bool_t(JET_BTAG[l] &(1<<2)) && JET_PT[l]>30 && abs(JET_ETA[l])<2.4){
       Nb_T++;
       Nb_M++;
       Nb_L++;
     }
     else if(Bool_t(JET_BTAG[l] &(1<<1)) && JET_PT[l]>30 && abs(JET_ETA[l])<2.4){
       Nb_M++;
       Nb_L++;
     }
     else if(Bool_t(JET_BTAG[l] &(1<<0)) && JET_PT[l]>30 && abs(JET_ETA[l])<2.4){
       Nb_L++;
     }
     else continue;
   }
...
//
    ...
    Int_t etrack_num=0;
    for(int l=0;l<nElec;l++){
      if(ELEC_HOE[l]<1 && ( ELEC_PT[l]<5 || ELEC_PT[l]/ELEC_SUMPT[l]<5)) continue;
...
////------------------calculate the invariant masses-------------------------
    ...
    delta_m[0][0]=abs(aux1.M()-aux2.M());
    delta_m[0][1]=(aux1.M()+aux2.M())/2.0;
    delta_R[0][0]=abs(b_vector[0].DeltaR(b_vector[1]));
    delta_R[0][1]=abs(b_vector[2].DeltaR(b_vector[3]));

    aux1=b_vector[0]+b_vector[2];
    aux2=b_vector[1]+b_vector[3];
    delta_m[1][0]=abs(aux1.M()-aux2.M());
    delta_m[1][1]=(aux1.M()+aux2.M())/2.0;
    delta_R[1][0]=abs(b_vector[0].DeltaR(b_vector[2]));
    delta_R[1][1]=abs(b_vector[1].DeltaR(b_vector[3]));

    aux1=b_vector[0]+b_vector[3];
    aux2=b_vector[2]+b_vector[1];
    delta_m[2][0]=abs(aux1.M()-aux2.M());
    delta_m[2][1]=(aux1.M()+aux2.M())/2.0;
    delta_R[2][0]=abs(b_vector[0].DeltaR(b_vector[3]));
    delta_R[2][1]=abs(b_vector[2].DeltaR(b_vector[1]));
